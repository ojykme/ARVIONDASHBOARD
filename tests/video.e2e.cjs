...
assert.equal(ctx.isAnimatedImageMedia({contentType:'image/gif'}),true);
assert.equal(ctx.isAnimatedImageMedia({url:'https://example.test/a.png'}),false);
...
assert.equal(await page.locator('#syncMediaPlayback').isEnabled(),true);
assert.equal(await page.locator('#syncPreview').isEnabled(),true);
assert.equal(await page.locator('#compareMode').isEnabled(),true);
assert.equal(await page.locator('#compareMode option[value="slider"]').isDisabled(),false);
assert.equal(await page.locator('#loopPlayback').isChecked(),false);
await page.locator('#loopPlayback').check();
assert.equal(await page.locator('video').evaluateAll(videos=>videos.every(video=>video.loop)),true);
assert.equal(JSON.parse(await page.evaluate(()=>sessionStorage.getItem('fixturePreferences'))).loopPlayback,true);
await page.locator('video').evaluateAll(videos=>videos.forEach(v=>v.muted=true));
await page.locator('#syncMediaPlayback').click();
await page.waitForFunction(()=>[...document.querySelectorAll('video')].every(v=>v.currentTime>0.2));
const offsets=await page.locator('video').evaluateAll(videos=>Math.abs(videos[0].currentTime-videos[1].currentTime));
assert.ok(offsets<0.15,`videos should start together; offset=${offsets}`);
